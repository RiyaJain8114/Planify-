...
const EventRecommendations: React.FC<EventRecommendationsProps> = ({
  ...
  const recommendations = React.useMemo(() => {
    ...
    const matchingEvents = upcomingEvents.filter(event => {
      ...
      return (
        (currentEventCategory && event.category === currentEventCategory) ||
        (currentEventCity && eventCity && currentEventCity === eventCity)
      );
    });
    
    if (matchingEvents.length === 0) {
      console.log("No matching events found, returning 2 random upcoming events");
      // If no matching events, just return a couple of random upcoming events
      return upcomingEvents.slice(0, maxRecommendations);
    }
                  ...
                  <Box sx={{ mt: 2, display: 'flex', alignItems: 'center' }}>
                    <Typography variant="body2" fontStyle="italic" color="text.secondary">
                      {event.category === currentEvent.category ? 
                        `Same category as ${currentEvent.title}` : 
                        `Event in same location`}
                    </Typography>
                  </Box>
                </CardContent>
